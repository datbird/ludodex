...
games already cached, so re-runs only fetch what's new.
  ...
  python3 ss_scrape.py --status        # just show your tier + quota
  python3 ss_scrape.py                  # scrape until daily cap (or all done)
  python3 ss_scrape.py --limit 50       # cap this run
  python3 ss_scrape.py --all            # re-scrape (ignore cache)
"""
import json
import os
import sqlite3
