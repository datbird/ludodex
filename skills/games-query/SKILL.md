...
- `sources(game_id, source, platform, source_id, title_raw, detail)` —
  `source` ∈ emulation|steam|gog|epic; emulation `platform` = system (psx, snes…);
...
recall. `sources_summary` e.g. `emulation:psx,sega saturn; steam; epic`.
...
sqlite3 -column -header "$DB" "SELECT canonical_title, sources_summary FROM games WHERE has_emulation=1 AND (has_steam=1 OR has_gog=1 OR has_epic=1);"
# Counts per source
sqlite3 -column -header "$DB" "SELECT SUM(has_emulation) emu, SUM(has_steam) steam, SUM(has_gog) gog, SUM(has_epic) epic, COUNT(*) total FROM games;"
```
