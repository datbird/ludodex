...
#
#   in-container:  docker exec -i ludodex bash /app/scripts/run_tests.sh
#   on a checkout: ./scripts/run_tests.sh  (tests needing fastapi will report an error)
set -u

DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"   # repo root, one up from scripts/
TESTS="$DIR/tests"
