...
Self-healing: --all ignores the caches and redoes everything.
  ...
  python3 ludodex/igdb_enrich.py            # incremental (resolve new, refresh stale)
  python3 ludodex/igdb_enrich.py --all      # ignore caches, re-resolve + re-fetch all
  python3 ludodex/igdb_enrich.py --limit N  # cap name-searches this run (testing)
"""
import json
import os
import re
...
def era_reheal(argv):
    ...
    media index so its wrong cover/background drops on the next media rebuild.
      ...
      python3 ludodex/igdb_enrich.py --era-reheal            # heal + purge
      python3 ludodex/igdb_enrich.py --era-reheal --dry-run  # report only, change nothing
    """
    dry = "--dry-run" in argv
    if not config.metadata_enabled("igdb"):
