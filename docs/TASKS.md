...
| 20 | **Collection members get no ingest** | `materialize_members` creates a catalog ROW and stops — no identity match, no media fetch, no attributes (`n_kinds=0` is hardcoded in the INSERT; `game_key` only reads resolutions that ALREADY exist). Live proof: `halo combat evolved anniversary` has 0 attributes, 0 media, no igdb_resolution, while its parent `halo the master chief collection` has 57 attributes and 75 media rows. The gap is invisible most of the time — all 112 members are materialize-created, yet 94 carry media because their norm_key already held data from earlier catalog history (matched 5.6 d before the collection was recorded). **Fix:** after materialize, feed the created keys through the normal identity + media path at the tier the PARENT was ingested at. Must be a phase AFTER materialize, not inside it, or §13's no-rebuild property dies. Scoped to the created set, hard-capped, no cascade. |
| 21 | **Every provider is a provider** (decision 2026-08-01) | **A MATCH IS NOT AN INGEST.** Every configured provider is matched for every game, whether or not any metadata or media is ever taken from it — the match is what makes a later on-demand pull possible. This REVERSES two recorded decisions: "SGDB stays art-only, it has no identity" (tiered-ingest spec) and ScreenScraper's retro-only framing. Three pieces: **(a)** SS eligibility follows `games.platform`, not `sources.source` — the gate is `s.source IN ('emulation','archive')`, and the live catalog's only source is `steam` (1624/1624 owned), so SS is currently unreachable for **every game in the library**, not just members; **(b)** SS gets an identity pass (it has a search API and working creds; there is no `ss_resolution` table today); **(c)** SGDB gets an identity pass and its remote source enabled (`media_steamgriddb_enabled` defaults to `0` — note it is NOT a duplicate of `media_steamgrid_enabled=1`, which is LOCAL Steam grid art from an installed client). **Throughput constraint:** SS allows ~100–200 calls/day (`screenscraper_daily_margin=200`, `screenscraper_devid` empty), so a 2208-game sweep is 11–22 days — this must be a **resumable backfill queue** that survives restarts and reports progress, never a blocking job. Chasing a real SS devid raises that ceiling. Link rendering is already done (`_provider_page_url`), so both light up the moment they record identities. |
